...
pub async fn run_main(cli: Cli, arg0_paths: Arg0DispatchPaths) -> anyhow::Result<()> {
    ...
    let SharedCliOptions {
        ...
        oss_provider,
        config_profile,
        permission_prompt_tool: permission_prompt_tool_cli_arg,
    ...
    let permission_prompt_tool = permission_prompt_tool_cli_arg.or(pop_string_cli_override(
    ...
    let model_provider = if oss {
        ...
        if let Some(provider) = resolved {
            ...
            return Err(anyhow::anyhow!(
                "No default OSS provider configured. Use --local-provider=provider or set oss_provider to one of: {OLLAMA_OSS_PROVIDER_ID} in config.toml"
            ));
        }
    } else {
