...
#[tokio::test]
async fn mcp_startup_resets_status_elapsed_when_agent_turn_is_already_running() {
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    ...
    chat.bottom_pane
        .force_status_elapsed_for_test(std::time::Duration::from_secs(18));
    ...
    assert_eq!(chat.bottom_pane.status_elapsed_seconds_for_test(), Some(0));
}
