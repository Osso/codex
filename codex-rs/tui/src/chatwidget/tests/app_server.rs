...
#[tokio::test]
async fn replayed_collab_tool_state_does_not_mark_agent_running() {
    let (mut chat, mut rx, _ops) = make_chatwidget_manual(/*model_override*/ None).await;
    let sender_thread_id = ThreadId::new();
    let spawned_thread_id = ThreadId::new();
    chat.set_collab_agent_metadata(
    ...
    assert!(
        chat.running_collab_agents.is_empty(),
        "replayed history must not repopulate the live footer agent cache"
    );
}

#[tokio::test]
async fn live_interrupted_collab_tool_state_clears_running_agent() {
    let (mut chat, mut rx, _ops) = make_chatwidget_manual(/*model_override*/ None).await;
    let sender_thread_id = ThreadId::new();
    let spawned_thread_id = ThreadId::new();
    chat.set_collab_agent_metadata(
    ...
    assert!(!chat.running_collab_agents.is_empty());
    ...
    assert!(
        chat.running_collab_agents.is_empty(),
        "interrupted agents should not remain in the live running-agent cache"
    );
}
