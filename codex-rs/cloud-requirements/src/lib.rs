...
impl CloudRequirementsService {
    ...
    async fn fetch(&self) -> Result<Option<ConfigRequirementsToml>, CloudRequirementsLoadError> {
        ...
        self.fetch_with_retries(auth, chatgpt_user_id.as_deref(), account_id.as_deref())
            .await
    }
    ...
    async fn refresh_cache(&self) -> bool {
        ...
        if let Err(err) = self
            .fetch_with_retries(auth, chatgpt_user_id.as_deref(), account_id.as_deref())
            .await
        {
            tracing::error!(
                path = %self.cache_path.display(),
...
mod tests {
    ...
    impl RequirementsFetcher for TokenFetcher {
        async fn fetch_requirements(
            ...
            self.request_count.fetch_add(1, Ordering::SeqCst);
            if auth.get_token().as_deref() == Ok(self.expected_token.as_str()) {
