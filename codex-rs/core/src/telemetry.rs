//! No-op telemetry stubs replacing `codex-otel`.
//!
//! All metric emission is dropped at compile time.
...
// Re-export sanitize_metric_tag_value from its actual source so callers
// that used `codex_otel::sanitize_metric_tag_value` still compile.
pub use codex_utils_string::sanitize_metric_tag_value;

// ---------------------------------------------------------------------------
// Metric name constants (stubs — values unused without a real exporter)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// MetricsClient / MetricsConfig stubs (used in test helpers)
// ---------------------------------------------------------------------------
#[derive(Debug, Clone)]
...
impl std::fmt::Display for MetricsError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "metrics disabled")
    }
}

// ---------------------------------------------------------------------------
// SessionTelemetry (no-op)
// ---------------------------------------------------------------------------
...
impl SessionTelemetry {
    ...
    pub fn tool_result_with_tags<A, B, C, D, E, F, G, H>(
        &self,
        _tool_name: A,
        _call_id: B,
        _log_payload: C,
    ...
    pub async fn log_tool_result_with_tags<A, B, C, D, E, F, Fut>(
        &self,
        _tool_name: A,
        _call_id: B,
        _log_payload: C,
...
// ---------------------------------------------------------------------------
// OtelProvider stub (returned from build_provider callers)
// ---------------------------------------------------------------------------
...
pub fn start_global_timer(_name: &str, _tags: &[(&str, &str)]) -> Result<Timer, MetricsError> {
    Err(MetricsError)
}

// ---------------------------------------------------------------------------
// RuntimeMetricTotals / RuntimeMetricsSummary stubs (used by tui)
// ---------------------------------------------------------------------------
