...
/// Structured request/auth fields that can be emitted as tracing tags.
...
pub fn emit_feedback_request_tags(tags: &FeedbackRequestTags<'_>) {
    let s = FeedbackRequestSnapshot::from_tags(tags);
    tracing::info!(
        target: "feedback_tags",
        endpoint = tracing::field::debug(s.endpoint),
        auth_header_attached = tracing::field::debug(s.auth_header_attached),
        auth_header_name = tracing::field::debug(s.auth_header_name),
        auth_mode = tracing::field::debug(s.auth_mode),
        auth_retry_after_unauthorized = tracing::field::debug(&s.auth_retry_after_unauthorized),
        auth_recovery_mode = tracing::field::debug(s.auth_recovery_mode),
        auth_recovery_phase = tracing::field::debug(s.auth_recovery_phase),
        auth_connection_reused = tracing::field::debug(&s.auth_connection_reused),
        auth_request_id = tracing::field::debug(s.auth_request_id),
        auth_cf_ray = tracing::field::debug(s.auth_cf_ray),
        auth_error = tracing::field::debug(s.auth_error),
        auth_error_code = tracing::field::debug(s.auth_error_code),
        auth_recovery_followup_success = tracing::field::debug(&s.auth_recovery_followup_success),
        auth_recovery_followup_status = tracing::field::debug(&s.auth_recovery_followup_status),
    );
}
