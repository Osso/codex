...
fn exec_output_text(output: &ExecCommandToolOutput) -> String {
    String::from_utf8_lossy(&output.raw_output).to_string()
}
...
async fn guardian_allows_shell_additional_permissions_requests_past_policy_validation() {
        ...
        .handle(ToolInvocation {
            ...
            payload: ToolPayload::Function {
                arguments: serde_json::json!({
                    "cmd": "echo hi",
                    "workdir": turn_context.cwd.to_string_lossy().to_string(),
    ...
    let output = resp.expect("expected Ok result");
    assert_eq!(output.exit_code, Some(0));
    assert!(exec_output_text(&output).contains("hi"));
}
...
async fn guardian_subagent_does_not_inherit_parent_exec_policy_rules() {
    ...
    let CodexSpawnOk { codex, .. } = Codex::spawn(CodexSpawnArgs {
        ...
        parent_rollout_thread_trace: crate::rollout_trace::ThreadTraceContext::disabled(),
