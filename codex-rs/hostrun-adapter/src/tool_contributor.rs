...
- `tools.ssh({ host, user, port, password, passwordMode }).run(cli.hostname())` runs a `cli.*` command builder remotely through OpenSSH and captures stdout/stderr by default. Use `.cli(command)` instead of `.run(command)` when you want to choose output handling, e.g. `tools.ssh({ host }).cli(cli.cat('/etc/os-release')).text()`. For Windows remotes, compose with `tools.powershell(script)`, which runs `powershell -NoProfile -EncodedCommand ...` and avoids nested quoting problems with paths containing spaces: `tools.ssh({ host: 'desktop' }).run(tools.powershell(\"Test-Path 'C:\\\\World of Warcraft\\\\_retail_\\\\Interface\\\\AddOns'\"))`. Connections are multiplexed by default (`ControlMaster=auto`, `ControlPersist=120s`): the first command opens a master connection and later commands to the same host reuse it without a new handshake. Store the client once and reuse it for every follow-up remote command instead of reconnecting or shelling out to `ssh`: `ctx.remote = tools.ssh({ host: 'router', user: 'root' }); ctx.remote.run(cli.uptime()); ctx.remote.cli(cli.cat('/etc/os-release')).text();`. Key-based connections also set `BatchMode=yes` so they fail fast instead of hanging on interactive prompts. Pass `multiplex: false` or `batchMode: false` to opt out, and explicit `options: [...]` entries override the matching default. Password auth is only enabled when `passwordMode: 'plain'` is explicit; this uses `sshpass -e`, redacts `SSHPASS` from approval metadata, and skips `BatchMode`.
- `tools.tmux` wraps common tmux session and pane actions through normal approval-gated command builders. Use `tools.tmux.open('work', { cwd: '/repo', command: 'zsh' })`, `tools.tmux.send('work', 'cargo test')`, `tools.tmux.capture('work', { start: -80 }).stdout`, `tools.tmux.run('work', 'ls -la')`, and `tools.tmux.close('work')`. `send()` uses `send-keys -l` plus `Enter` by default; pass `{ enter: false }` to leave text at the prompt or `{ literal: false }` for tmux key names. `capture()` reads existing pane output without sending anything. `run()` sends a shell command wrapped with start/end markers, polls `capture-pane`, and returns `{ stdout, exitCode, timedOut }`.
...
- `tools.git.status({ cwd })` returns `git status --short --branch` text by default. Pass `short: false` or `branch: false` to omit either flag.
...
mod tests {
    ...
    fn feature_gated_install_contributes_tool_and_instructions_when_enabled() {
        ...
        assert!(fragments[0].text().contains("ControlPersist=120s"));
        assert!(fragments[0].text().contains("ctx.remote = tools.ssh"));
        assert!(fragments[0].text().contains("BatchMode=yes"));
        ...
        assert!(
            fragments[0]
                .text()
                .contains("Kubernetes secret plus rclone")
        );
