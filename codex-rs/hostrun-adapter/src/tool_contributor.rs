...
- `tools.tmux` wraps common tmux session and pane actions through normal approval-gated command builders. Use `tools.tmux.open('work', { cwd: '/repo', command: 'zsh' })`, `tools.tmux.send('work', 'cargo test')`, `tools.tmux.capture('work', { start: -80 }).stdout`, and `tools.tmux.close('work')`. `send()` uses `send-keys -l` plus `Enter` by default; pass `{ enter: false }` to leave text at the prompt or `{ literal: false }` for tmux key names.
