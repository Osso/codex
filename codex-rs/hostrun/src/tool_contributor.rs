...
- `fs.write(path, content)`, `fs.read(path)`, `fs.open(path, options)`, `fs.glob(pattern, options)`, `fs.exists(path)`, and `fs.remove(path)` request approval-gated host file operations. `fs.open` parses JSON, JSONL, YAML, CSV, and TSV from the filename extension unless `options.format` is supplied.
