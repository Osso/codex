...
- `run.<program>(...args)` executes a host command without stdout/stderr capture by default, e.g. `run.dmidecode()` or `run.git('status', '--short')`. `run` is not a shell parser: use `run.dmidecode('-t', 'system')`, not `run('dmidecode -t system')`. For privileged commands use `run.sudo('dmidecode', '-t', 'system')` or `cli.sudo('dmidecode', '-t', 'system')`; Hostrun maps `sudo` to `authsudo`.
