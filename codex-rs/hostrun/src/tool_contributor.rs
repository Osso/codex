...
- `tools.sudo(commandBuilder)` wraps a `cli.*` command builder with `authsudo` for privileged commands. Example: `tools.sudo(cli.dmidecode('-t', 'system')).run()`. `cli.sudo(...)` and `run.sudo(...)` still invoke the `sudo` binary literally.
