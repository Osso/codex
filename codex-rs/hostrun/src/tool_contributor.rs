...
- `tools.sudo(program, ...args)` builds an `authsudo` command builder for privileged commands. Example: `tools.sudo('dmidecode', '-t', 'system').complete()`. `cli.sudo(...)` and `run.sudo(...)` still invoke the `sudo` binary literally.
