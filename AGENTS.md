...
- When you cannot make that API change and still need a small positional-literal callsite in Rust, use an exact `/*param_name*/` comment before opaque literal arguments such as `None`, booleans, and numeric literals when passing them by position. Do not add these comments for string or char literals unless the comment adds real clarity. The parameter name in the comment must exactly match the callee signature.
...
- Prefer `codex_utils_cargo_bin::cargo_bin("...")` over `assert_cmd::Command::cargo_bin(...)` or `escargot` when tests need to spawn first-party binaries; it resolves absolute paths that remain stable after `chdir`.
- When locating fixture files or test resources, prefer `codex_utils_cargo_bin::find_resource!` over `env!("CARGO_MANIFEST_DIR")`.
